...
int main()
{
	RenderWindow app(VideoMode(width, height), "Safari Auto!");
	...
	public:
		...
		//Overloaded constructor that accepts an argument for gallons. It can hold no more than 15 gallons.
		FuelGauge(int g)
		{
			if (g <= 15)
				gallons = g;
		...
		//The incrementGallons function increments the value of the gallons member, if it is less than 15. The car holds a maximum of 15 gallons.
		void incrementGallons()
		{
			...
			// The decrementGallons function decrements the value of the gallons member, if it is greater than 0. void decrementGallons() 
		}
	...
	private:
		...
		//Milleage set-point to remember when the FuelGauge gallons were decremented
		int setPoint;
		...
		//Member Functions
	public:
		//The constructor accepts arguments for the mileage and the fuel gauge
		Odometer(int m, FuelGauge *fp)
		{
			mileage = m; setPoint = m; fuelGauge = fp;
		}
		//Accessor for the milleage
		int getMileage()
		{
			return mileage;
		}
		//The incrementMiles function increments the value of the mileage member
		void incrementMiles()
		{
			//Increment the mileage, but rollover if we go past 999,999
			if (mileage < 999999) mileage++; else mileage = 0; //See, if we have burned a gallon of gas. This happens every 24 miles. If setPoint is greater than mileage, then the odometer has rolled over if (setPoint > mileage)
			{
	...
	for (int i = 0; i < 15; i++) fuel.incrementGallons(); //Drive the car until it is out of gas
